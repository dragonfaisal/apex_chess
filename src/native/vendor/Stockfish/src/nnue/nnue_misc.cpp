...
void hint_common_parent_position(const Position&    pos,
    ...
    if (Eval::use_smallnet(pos))
        ...
        networks.big.hint_common_access(pos, &caches.small);
}
...
trace(Position& pos, const Eval::NNUE::Networks& networks, Eval::NNUE::AccumulatorCaches& caches) {
    ...
    auto [psqt, positional] = networks.big.evaluate(pos, &caches.small);
    ...
    for (File f = FILE_A; f <= FILE_H; ++f)
        for (Rank r = RANK_1; r <= RANK_8; ++r)
        {
            ...
            if (pc != NO_PIECE && type_of(pc) != KING)
            {
                ...
                std::tie(psqt, positional) = networks.big.evaluate(pos, &caches.small);
    ...
    auto t = networks.big.trace_evaluate(pos, &caches.small);
