...
const debugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportExitSuccess =
    0;
const debugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportExitUsage =
    64;
const debugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportExitBlockedStrict =
    68;
const debugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportExitUnsafePolicy =
    69;

enum DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportFormat {
  markdown('markdown'),
  json('json');

  const DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportFormat(
    this.wire,
  );
...
class DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportCommandResult {
  const DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportCommandResult({
  ...
  final int exitCode;
  final DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportFormat
  format;
...
class _CommandRequest {
  ...
  const _CommandRequest.invalid(this.failure)
    : isValid = false,
      format =
          DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportFormat
              .markdown,
      mode =
...
void main(List<String> args) {
  final result =
      runDebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportCommand(
        args: args,
      );
...
runDebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportCommand({
  ...
  DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnostic
      diagnostic =
      const DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnostic(),
}) {
  final request = _parseArgs(args);
  if (!request.isValid) {
    return DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportCommandResult(
      exitCode:
...
int _exitCode(
  ...
  if (hasTextLeak || result.hasUnsafePolicyViolation) {
    return debugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportExitUnsafePolicy;
  }
  if (request.strict &&
      (result.blockerCount > 0 ||
          ...
          !result.safeForPhase34O ||
          result.nextRecommendation !=
              'implementControlledAnalyzerAdapterRuntimeInputPreflightPatch')) {
    return debugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportExitBlockedStrict;
  }
  return debugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportExitSuccess;
}
...
_formatByWire(String wire) {
  for (final format
      in DebugOnlyBridgeAnalyzerAdapterDisabledRuntimeExecutionSeamProbeDiagnosticReportFormat
          .values) {
...
String _usage(String failure) {
  return [
    ...
    'usage: dart run tool/debug_only_bridge_analyzer_adapter_disabled_runtime_execution_seam_probe_diagnostic_report.dart [--format=markdown|json] [--strict] [--safe-demo] [--include-warnings] [--disabled-seam-probe-diagnostic=default|all-safe|request|response|attempt|boundaries|blocked-reasons|denied|proof|recommendation]',
    'defaults: --format=markdown --safe-demo --include-warnings --disabled-seam-probe-diagnostic=default',
  ].join('\n');
}

const _formatFlag = '--format=';
const _diagnosticFlag = '--disabled-seam-probe-diagnostic=';
