...
const debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitSuccess =
    0;
const debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitUsage =
    64;
const debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitBlockedStrict =
    68;
const debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitUnsafePolicy =
    69;

enum DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportFormat {
  markdown('markdown'),
  json('json');

  const DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportFormat(
    this.wire,
  );
...
class DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportCommandResult {
  const DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportCommandResult({
  ...
  final int exitCode;
  final DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportFormat
  format;
  final DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanSection
  section;
  final bool strict;
  final bool safeDemo;
  ...
  final DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanResult?
  result;
...
class _CommandRequest {
  ...
  const _CommandRequest.invalid(this.failure)
    : isValid = false,
      format =
          DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportFormat
              .markdown,
      section =
...
void main(List<String> args) {
  final result =
      runDebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportCommand(
        args: args,
      );
...
runDebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportCommand({
  ...
  if (!request.isValid) {
    return DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportCommandResult(
      exitCode:
  ...
  final validationResult = validation.evaluate();
  final result = actionPlan.evaluate(validationResult: validationResult);
...
int _exitCode(
  ...
  if (hasTextLeak) {
    return debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitUnsafePolicy;
  }
  if (request.strict && result.hasUnsafePolicyViolation) {
    return debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitBlockedStrict;
  }
  if (result.hasUnsafePolicyViolation) {
    return debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitUnsafePolicy;
  }
  return debugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanExitSuccess;
}
...
_formatByWire(String wire) {
  for (final format
      in DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanReportFormat
          .values) {
...
_sectionByWire(String wire) {
  for (final section
      in DebugOnlyBridgeAnalyzerAdapterPrototypeSelectedDiagnosticActionPlanSection
          .values) {
...
String _usage(String failure) {
  return [
    ...
    'usage: dart run tool/debug_only_bridge_analyzer_adapter_prototype_selected_diagnostic_action_plan_report.dart [--format=markdown|json] [--strict] [--safe-demo] [--include-warnings] [--section=all|actions|warning|proof|guards|blocked|recommendation]',
