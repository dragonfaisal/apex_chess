...
class UsernameValidationState {
  ...
  bool get isResolved =>
      phase == ValidationPhase.result && existence != UsernameExistence.unknown;
  bool get isGreen => isResolved && existence == UsernameExistence.exists;
  bool get isRed => isResolved && existence == UsernameExistence.missing;
  bool get isSpinning =>
      phase == ValidationPhase.debouncing || phase == ValidationPhase.loading;
}

class UsernameValidationController
    extends ValueNotifier<UsernameValidationState> {
  ...
  /// Called on every keystroke + source toggle. Resets the pill to idle until
  /// the query is at least three characters, then schedules a check.
  ...
  Future<void> _fire(int gen, String source, String username) async {
    ...
    final result = await _validator.check(source: source, username: username);
    if (gen != _generation) return;
    value = UsernameValidationState(
