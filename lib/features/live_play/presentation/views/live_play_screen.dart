...
class LivePlayScreen extends ConsumerStatefulWidget {
  const LivePlayScreen({super.key});

  @override
  ConsumerState<LivePlayScreen> createState() => _LivePlayScreenState();
}

class _LivePlayScreenState extends ConsumerState<LivePlayScreen> {
  bool _recordedLiveStart = false;

  @override
  Widget build(BuildContext context) {
    ref.listen<LivePlayState>(livePlayProvider, (previous, next) {
      if (_recordedLiveStart) return;
      if (previous?.lastMove == null && next.lastMove != null) {
        _recordedLiveStart = true;
        unawaited(
    ...
    final isBrilliant = s.moveAnalysis?.quality == MoveQuality.brilliant;
        ...
        child: SafeArea(
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              final maxBoardHeight = (constraints.maxHeight * 0.62).clamp(
                240.0,
                560.0,
              );
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context, LivePlayState s) {
    return AppBar(
      ...
      title: Row(
        ...
        children: [
          Icon(
            Icons.auto_awesome,
            color: ApexColors.sapphireBright.withValues(alpha: 0.85),
            size: 20,
          ),
...
class _CoachDashboard extends StatelessWidget {
  ...
  Widget _buildContent() {
    ...
    if (isStalemate) {
      return _statusRow('½', 'STALEMATE', ApexColors.textTertiary);
    }
    if (isDraw) {
      return _statusRow('½', 'DRAW', ApexColors.textTertiary);
    }
