...
import '../../../../tool/debug_only_bridge_analyzer_adapter_controlled_runtime_input_preflight_report.dart';

void main() {
  group(
    'Debug-Only Bridge Analyzer Adapter controlled runtime input preflight report command',
    () {
      test('command file exists', () {
        expect(
          File(
            'tool/debug_only_bridge_analyzer_adapter_controlled_runtime_input_preflight_report.dart',
          ).existsSync(),
          isTrue,
        );
      });
      ...
      test('markdown report succeeds', () {
        ...
        expect(result.result!.safeForPhase34P, isTrue);
        expect(
        ...
        expect(
          result.stdoutText,
          contains('## Runtime Input Preflight Check Summary'),
        );
        expect(
          result.stdoutText,
          contains('## Runtime Input Preflight Decision Summary'),
        );
            ...
            contains(
              section ==
                      DebugOnlyBridgeAnalyzerAdapterControlledRuntimeInputPreflightReportSection
                          .all
                  ? '# Controlled Analyzer Adapter Runtime Input Preflight'
                  : section.wire,
            ),
          );
          expect(
      ...
      test('source imports remain command-only and integration-free', () {
        final modelSource = File(
          'lib/features/pgn_review/application/debug_only_bridge_analyzer_adapter_controlled_runtime_input_preflight.dart',
        ).readAsStringSync();
...
_run({List<String> args = const <String>[]}) {
  return runDebugOnlyBridgeAnalyzerAdapterControlledRuntimeInputPreflightReportCommand(
    args: args,
  );
}
