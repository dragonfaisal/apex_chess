...
import '../../../../tool/internal_non_label_signal_profile_report.dart';

void main() {
  group('Internal Non-Label Signal Profile report command', () {
    test('command file exists', () {
      expect(
        File('tool/internal_non_label_signal_profile_report.dart').existsSync(),
        true,
      );
    });
    ...
    test('default output is safe-demo markdown and succeeds', () {
      ...
      expect(result.exitCode, internalNonLabelSignalProfileReportExitSuccess);
      expect(result.format, InternalNonLabelSignalProfileReportFormat.markdown);
      expect(result.strict, isFalse);
      expect(result.safeDemo, isTrue);
      expect(result.includePartial, isFalse);
      expect(
        result.stdoutText,
        contains('# Internal Non-Label Signal Profile'),
      );
    ...
    test('json output is valid and deterministic', () {
      ...
      expect(first.exitCode, internalNonLabelSignalProfileReportExitSuccess);
    ...
    test('strict mode succeeds for safe demo', () {
      ...
      expect(result.exitCode, internalNonLabelSignalProfileReportExitSuccess);
      expect(result.result!.guardAllowed, isTrue);
      expect(result.result!.hasUnsafeSignalProfilePolicyViolation, isFalse);
    });
    ...
    test('safe-demo and include-partial flags are accepted', () {
      ...
      expect(
        result.result!.signalCount,
        InternalNonLabelSignalId.values.length,
      );
    });

    test('strict mode fails when profile is blocked without unsafe output', () {
      final result = _run(
        args: const ['--strict'],
        profile: const _FakeProfile(_blockedResult),
      );

      expect(
        result.exitCode,
        internalNonLabelSignalProfileReportExitBlockedStrict,
      );
    });
    ...
    test('strict mode fails on unsafe output-policy seam', () {
      final result = _run(
        args: const ['--strict'],
        profile: const _FakeProfile(_unsafeOutputResult),
      );

      expect(
        result.exitCode,
        internalNonLabelSignalProfileReportExitUnsafePolicy,
      );
    });
    ...
    test('unknown flag exits usage error', () {
      ...
      expect(result.exitCode, internalNonLabelSignalProfileReportExitUsage);
...
InternalNonLabelSignalProfileReportCommandResult _run({
  List<String> args = const [],
  InternalNonLabelSignalProfilePrototype profile =
      const InternalNonLabelSignalProfilePrototype(),
}) {
  return runInternalNonLabelSignalProfileReportCommand(
    args: args,
    profile: profile,
  );
}

class _FakeProfile implements InternalNonLabelSignalProfilePrototype {
  const _FakeProfile(this.result);

  final InternalNonLabelSignalProfileResult result;

  @override
  InternalNonLabelSignalProfileValidator get validator =>
...
const _blockedResult = InternalNonLabelSignalProfileResult(
  status: InternalNonLabelSignalProfileStatus.blockedByDesign,
  scoringDesignStatus: InternalNonLabelScoringDesignStatus.blockedByMatrix,
  ...
  guardAllowed: false,
  signalCount: 0,
  signals: <InternalNonLabelSignalProfileEntry>[],
  validationFindings: <InternalNonLabelSignalProfileValidationFinding>[],
  warnings: <String>[],
  failures: <String>['scoring design blocked request'],
...
const _unsafeOutputResult = InternalNonLabelSignalProfileResult(
  status: InternalNonLabelSignalProfileStatus.profileReadyInternalOnly,
  scoringDesignStatus:
      InternalNonLabelScoringDesignStatus.designReadyInternalOnly,
  ...
  failures: <String>[],
  nextRecommendedPhase:
      InternalNonLabelSignalProfileNextPhase.guardedInternalSignalExperiments,
  provenAndroidCaseIds: <String>[],
  unprovenAndroidCaseIds: <String>[],
...
String _commandSource() {
  return File(
    'tool/internal_non_label_signal_profile_report.dart',
  ).readAsStringSync();
}
