...
import '../../../../tool/debug_only_bridge_analyzer_adapter_prototype_contract_design_report.dart';

void main() {
  group('Debug-Only Bridge analyzer adapter prototype contract design report', () {
    test('command file exists', () {
      expect(
        File(
          'tool/debug_only_bridge_analyzer_adapter_prototype_contract_design_report.dart',
        ).existsSync(),
        isTrue,
      );
    });
    ...
    test('default markdown command succeeds', () {
      ...
      expect(result.stdoutText, contains('## Contract Group Table'));
      expect(result.stdoutText, contains('## Contract Record Table'));
      expect(result.stdoutText, contains('## Allowed Internal Field Contract'));
      expect(result.stdoutText, contains('## Denied Field Contract'));
      expect(result.stdoutText, contains('## Blocked Boundary Contract'));
      ...
      expect(result.result!.safeForPhase33R, isTrue);
    ...
    test('JSON command succeeds and is parseable', () {
      ...
      expect(decoded['contractGroups'], isA<List<Object?>>());
      expect(decoded['contractRecords'], isA<List<Object?>>());
    });

    test('strict mode succeeds for safe demo contract design', () {
    ...
    test('usage rejects unknown flags', () {
      ...
      expect(
        result.exitCode,
        debugOnlyBridgeAnalyzerAdapterPrototypeContractDesignExitUsage,
      );
...
DebugOnlyBridgeAnalyzerAdapterPrototypeContractDesignReportCommandResult _run({
  List<String> args = const <String>[],
}) {
  return runDebugOnlyBridgeAnalyzerAdapterPrototypeContractDesignReportCommand(
    args: args,
  );
}
