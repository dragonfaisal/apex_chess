...
import '../../../../tool/debug_only_bridge_analyzer_adapter_prototype_skeleton_validation_report.dart';

void main() {
  group(
    'debug_only_bridge_analyzer_adapter_prototype_skeleton_validation_report',
    () {
      test('default markdown command succeeds', () {
        ...
        expect(result.result!.safeForPhase33W, isTrue);
        expect(result.stdoutText, contains('Validation Check Table'));
        expect(result.stdoutText, contains('Packet Validation Summary'));
        expect(result.stdoutText, contains('Record Role Validation Summary'));
        expect(result.stdoutText, contains('Policy Validation Summary'));
      ...
      test('source imports remain command-only and integration-free', () {
        final source = File(
          'tool/debug_only_bridge_analyzer_adapter_prototype_skeleton_validation_report.dart',
        ).readAsStringSync();
...
_run({List<String> args = const <String>[]}) {
  return runDebugOnlyBridgeAnalyzerAdapterPrototypeSkeletonValidationReportCommand(
    args: args,
  );
}
