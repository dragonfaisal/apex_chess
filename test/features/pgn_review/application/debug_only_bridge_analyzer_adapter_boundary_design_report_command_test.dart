...
import '../../../../tool/debug_only_bridge_analyzer_adapter_boundary_design_report.dart';

void main() {
  group('Debug-Only Bridge analyzer adapter boundary design report', () {
    test('command file exists', () {
      expect(
        File(
          'tool/debug_only_bridge_analyzer_adapter_boundary_design_report.dart',
        ).existsSync(),
        isTrue,
      );
    });
    ...
    test('default markdown command succeeds', () {
      ...
      expect(
        result.exitCode,
        debugOnlyBridgeAnalyzerAdapterBoundaryDesignExitSuccess,
      );
      expect(
        result.stdoutText,
        contains('# Debug-Only Bridge Analyzer Adapter Boundary Design'),
      );
      expect(result.stdoutText, contains('## Boundary Component Table'));
      expect(result.stdoutText, contains('## Boundary Record Table'));
      expect(
        result.stdoutText,
        contains('## Analyzer Input Candidate Summary'),
      );
      expect(result.stdoutText, contains('## Denied Field Boundary'));
      expect(
        result.stdoutText,
        contains('## Runtime, Scheduler, Persistence, And Engine Boundaries'),
      );
      expect(result.stdoutText, contains('## Android Proof Boundary'));
      expect(
        result.stdoutText,
        contains('validateDebugOnlyBridgeAnalyzerAdapterBoundaryDesign'),
      );
      expect(result.result!.safeForPhase33N, isTrue);
    ...
    test('JSON command succeeds and is parseable', () {
      ...
      expect(decoded['designStatus'], 'boundaryDesignReadyWithWarnings');
      ...
      expect(decoded['components'], isA<List<Object?>>());
      expect(decoded['boundaryRecords'], isA<List<Object?>>());
    });

    test('strict mode succeeds for safe demo design', () {
    ...
    test('usage rejects unknown flags', () {
      ...
      expect(
        result.exitCode,
        debugOnlyBridgeAnalyzerAdapterBoundaryDesignExitUsage,
      );
...
DebugOnlyBridgeAnalyzerAdapterBoundaryDesignReportCommandResult _run({
  List<String> args = const <String>[],
}) {
  return runDebugOnlyBridgeAnalyzerAdapterBoundaryDesignReportCommand(
    args: args,
  );
}
