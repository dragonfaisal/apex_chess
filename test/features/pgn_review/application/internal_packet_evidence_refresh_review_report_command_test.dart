...
import '../../../../tool/internal_packet_evidence_refresh_review_report.dart';

void main() {
  group('Internal Packet Evidence Refresh Review report command', () {
    test('command file exists', () {
      expect(
        File(
          'tool/internal_packet_evidence_refresh_review_report.dart',
        ).existsSync(),
        true,
      );
    });
    ...
    test('markdown works for safe demo', () {
      ...
      expect(
        result.exitCode,
        internalPacketEvidenceRefreshReviewReportExitSuccess,
      );
      expect(
        result.format,
      ...
      expect(
        result.stdoutText,
        contains('# Internal Packet Evidence Refresh Review'),
      );
      expect(result.stdoutText, contains('## Review Table'));
      ...
      expect(result.result!.safeForPhase32K, isTrue);
    });
    ...
    test('JSON works for safe demo', () {
      ...
      expect(decoded['totalRows'], 20);
      expect(decoded['validImprovedSupportCount'], 2);
      expect(decoded['validWarningLimitedCount'], 4);
    ...
    test('strict fails on unsafe seam', () {
      ...
      final result = _run(
        ...
        request: InternalPacketEvidenceRefreshReviewRequest(
      ...
      expect(
        result.exitCode,
        internalPacketEvidenceRefreshReviewReportExitUnsafePolicy,
      );
      expect(result.result!.hasUnsafeReviewPolicyViolation, isTrue);
    });
    ...
    test('safe-demo and include-warnings flags are accepted', () {
      ...
      expect(result.result!.validWarningLimitedCount, 4);
    });
    ...
    test('usage errors return usage exit code', () {
      ...
      expect(
        result.exitCode,
        internalPacketEvidenceRefreshReviewReportExitUsage,
      );
...
InternalPacketEvidenceRefreshReviewReportCommandResult _run({
  List<String> args = const <String>[],
  InternalPacketEvidenceRefreshReviewRequest? request,
}) {
  return runInternalPacketEvidenceRefreshReviewReportCommand(
    args: args,
    request: request,
  );
}
