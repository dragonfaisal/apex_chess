...
import '../../../../tool/debug_only_bridge_analyzer_adapter_prototype_contract_design_validation_report.dart';

void main() {
  group(
    'Debug-Only Bridge analyzer adapter prototype contract design validation report',
    () {
      test('command file exists', () {
        expect(
          File(
            'tool/debug_only_bridge_analyzer_adapter_prototype_contract_design_validation_report.dart',
          ).existsSync(),
          isTrue,
        );
      });
      ...
      test('default markdown command succeeds', () {
        ...
        expect(result.stdoutText, contains('## Contract Group Validation'));
        expect(
          result.stdoutText,
          contains('## Contract Record Validation Table'),
        );
        expect(
          result.stdoutText,
        ...
        expect(result.result!.safeForPhase33S, isTrue);
...
_run({List<String> args = const <String>[]}) {
  return runDebugOnlyBridgeAnalyzerAdapterPrototypeContractDesignValidationReportCommand(
    args: args,
  );
}
