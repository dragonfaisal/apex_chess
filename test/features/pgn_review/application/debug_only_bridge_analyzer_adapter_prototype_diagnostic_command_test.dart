...
void main() {
  group('Debug-Only Bridge Analyzer Adapter Prototype Diagnostic command', () {
    ...
    test('JSON command succeeds and is parseable', () {
      ...
      expect(
        decoded['runtimeInputEnvelopeActivationPreflight'],
        isA<Map<String, Object?>>(),
      );
    });
    ...
    test('disabled seam probe diagnostic JSON and strict modes succeed', () {
      final json = _run(
        args: const <String>[
          '--disabled-seam-probe-diagnostic=default',
          '--format=json',
        ],
      );
      final strict = _run(
      ...
      final seamProbeDiagnostic =
          decoded['disabledRuntimeExecutionSeamProbeDiagnostic']
              as Map<String, Object?>;
      final counts = seamProbeDiagnostic['counts'] as Map<String, Object?>;
      ...
      expect(
        json.disabledRuntimeExecutionSeamProbeDiagnosticMode!.wire,
        'default',
      );
      expect(
        json.disabledRuntimeExecutionSeamProbeDiagnostic!.status,
      ...
      expect(
        seamProbeDiagnostic['status'],
        'disabledAnalyzerAdapterRuntimeExecutionSeamProbeDiagnosticReadyWithWarnings',
      );
      expect(seamProbeDiagnostic['safeForPhase34O'], isTrue);
      expect(
        seamProbeDiagnostic['nextRecommendation'],
      ...
      expect(
        strict.disabledRuntimeExecutionSeamProbeDiagnostic!.safeForPhase34O,
        isTrue,
      );
    });

    test('runtime input preflight section JSON and strict modes succeed', () {
      final json = _run(
        args: const <String>[
          '--section=runtime-input-preflight',
          '--format=json',
        ],
      );
      final strict = _run(
        args: const <String>['--section=runtime-input-preflight', '--strict'],
      );
      final decoded = jsonDecode(json.stdoutText) as Map<String, Object?>;
      final runtimeInputPreflight =
          decoded['runtimeInputPreflight'] as Map<String, Object?>;
      final counts = runtimeInputPreflight['counts'] as Map<String, Object?>;
      ...
      expect(
        json.runtimeInputPreflight!.status,
      ...
      expect(
        runtimeInputPreflight['status'],
        'controlledAnalyzerAdapterRuntimeInputPreflightReadyWithWarnings',
      );
      expect(runtimeInputPreflight['safeForPhase34P'], isTrue);
      expect(
        runtimeInputPreflight['nextRecommendation'],
      ...
      expect(strict.runtimeInputPreflight!.safeForPhase34P, isTrue);
      expect(
        strict.stdoutText,
        contains('## Controlled Runtime Input Preflight'),
      );
    ...
    test(
      'runtime input envelope activation preflight section JSON and strict modes succeed',
      () {
        final json = _run(
          args: const <String>[
            '--section=runtime-input-envelope-activation-preflight',
        ...
        final decoded = jsonDecode(json.stdoutText) as Map<String, Object?>;
        final activationPreflight =
            decoded['runtimeInputEnvelopeActivationPreflight']
                as Map<String, Object?>;
        final counts = activationPreflight['counts'] as Map<String, Object?>;
        ...
        expect(
          json.runtimeInputEnvelopeActivationPreflight!.status,
        ...
        expect(
          activationPreflight['status'],
          'controlledAnalyzerAdapterRuntimeInputEnvelopeActivationPreflightReadyWithWarnings',
        );
        expect(activationPreflight['safeForPhase34T'], isTrue);
        expect(
          activationPreflight['nextRecommendation'],
        ...
        expect(
          strict.runtimeInputEnvelopeActivationPreflight!.safeForPhase34T,
          isTrue,
        );
        expect(
          strict.stdoutText,
    ...
    test('each disabled seam probe diagnostic mode succeeds through command', () {
      for (final mode
        ...
        expect(result.disabledRuntimeExecutionSeamProbeDiagnosticMode, mode);
        expect(
          result.disabledRuntimeExecutionSeamProbeDiagnostic!.safeForPhase34O,
          isTrue,
        );
        expect(
          result.stdoutText,
          contains('disabled seam probe diagnostic mode: ${mode.wire}'),
        );
      }
    });
    ...
    test('invalid section and format fail with usage error', () {
      ...
      expect(
        badDisabledSeamProbeDiagnostic.exitCode,
        debugOnlyBridgeAnalyzerAdapterPrototypeDiagnosticCommandExitUsage,
      );
      expect(
        badDisabledSeamProbeDiagnostic.commandFailure,
    ...
    test('output contains no raw engine spam or active product data', () {
      ...
      final disabledRuntimeExecutionSeamProbeDiagnostic = _run(
        args: const <String>['--disabled-seam-probe-diagnostic=default'],
      ).stdoutText;
      ...
      final runtimeInputEnvelopeActivationPreflight = _run(
        args: const <String>[
          '--section=runtime-input-envelope-activation-preflight',
        ],
      ).stdoutText;
