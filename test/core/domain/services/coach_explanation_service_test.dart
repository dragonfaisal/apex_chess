...
void main() {
  ...
  group('Rule 1 — Mate-delivered ply never reads as Blunder', () {
    test('Opponent mate move reads "Checkmate."', () {
      ...
      expect(cls.subline.toLowerCase(), contains('opponent delivered mate'));
  ...
  group('Rule 2 — Allowed-forced-mate blunder', () {
    test(
      'Blunder that allowed mate surfaces "Allowed forced mate" subline',
      () {
        final cls = svc.explain(
          CoachExplanationInput(
            move: _m(
        ...
        expect(cls.subline.toLowerCase(), contains('defensive resources'));
      },
    );
  });
  ...
  group('Rule 3 — Played == best → Top engine choice', () {
    test(
      'Played UCI matches best UCI → headline "Best", no better-move line',
      () {
        final cls = svc.explain(
          CoachExplanationInput(
            move: _m(
  ...
  group('Rule 4 — Needs Deep Scan flag on Quick-mode candidates', () {
    test('Quick mode + tactical best-move ply → needsDeepScan=true', () {
      ...
      expect(cls.subline.toLowerCase(), contains('deep scan recommended'));
    });
  ...
  group('Rule 6b — better arrow suppression', () {
    test('Played == best suppresses board better-move arrow', () {
      final move = _m(
        ply: 14,
        san: 'Bxf7+',
        uci: 'c4f7',
        bestUci: 'c4f7',
        bestSan: 'Bxf7+',
        classification: MoveQuality.inaccuracy,
      );

      expect(shouldShowBetterMoveArrowForTesting(move), isFalse);
    });
  });

  group(
    'Rule 7 — Better line suppressed on Brilliant/Great/Forced/Book/Best',
    () {
      for (final tier in [
  ...
  group('Rule 8 — Opening phase vs Book', () {
    test('Out-of-book early ply (ply<8, not inBook) appends Opening phase '
      ...
      expect(
        ...
        reason:
            'Non-book early plies must surface the Opening phase '
    ...
    test('In-book ply (inBook=true) reads "Book move." and never says '
      ...
      expect(
        cls.subline.toLowerCase().contains('opening phase'),
        isFalse,
